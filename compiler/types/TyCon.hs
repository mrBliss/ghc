...
module TyCon(
        ...
        isDictTyCon, dictTyConClass_maybe, dictTyConCo_maybe, dictTyConCo,
...
data TyCon
  ...
  --     - dictionary record type generated by a class declaration
  --     - boxed tuples
  --     - unboxed tuples
  --     - constraint tuples
...
data AlgTyConFlav
  ...
  -- | TODOT Dictionary type constructor of a class
  | DictTyCon
        Class                 -- INVARIANT: the classDictTyCon of this Class is the
                              -- current tycon
        TyConRepName          -- TODOT why do we need this?
        (CoAxiom Unbranched)  -- Coercion axiom to convert a dictionary type
                              -- to a class type,
                              -- e.g. C.Dict a :: * to C a :: Constraint.
...
instance Outputable AlgTyConFlav where
    ...
    ppr (DictTyCon cls _ _)         = text "Class parent" <+> ppr cls  -- TODOT
...
okParent tc_name (DictTyCon cls _ _)             = tc_name == tyConName (classDictTyCon cls)
...
-- Note that:
--
-- * Associated families and dictionary records are implicit, as they are
--   re-constructed from the class declaration in which they reside, and
--
...
-- | Is this 'TyCon' that of a dictionary?
isDictTyCon :: TyCon -> Bool
isDictTyCon (AlgTyCon {algTcParent = DictTyCon {}}) = True
isDictTyCon _                                       = False

-- | If this 'TyCon' is that for a class dictionary, return the class it is
-- for. Otherwise returns @Nothing@
dictTyConClass_maybe :: TyCon -> Maybe Class
dictTyConClass_maybe (AlgTyCon {algTcParent = DictTyCon clas _ _}) = Just clas
dictTyConClass_maybe _                                             = Nothing

-- | Extracts the coercion from a class dictionary to convert it to the class.
--
-- For example, to convert C.Dict a :: * to C a :: Constraint
--
-- If the supplied 'TyCon' is not of a class dictionary, return @Nothing@.
dictTyConCo_maybe :: TyCon -> Maybe (CoAxiom Unbranched)
dictTyConCo_maybe (AlgTyCon {algTcParent = DictTyCon _ _ co}) = Just co
