...
import {-# SOURCE #-}   HsExpr( HsExpr, LHsExpr, HsSplice, pprExpr,
                                pprSpliceDecl )
...
data ClsInstDecl pass
  ...
  | ClsInstExpr  -- TODOT ClsInstDecl having ClsInstDecl and ClsInstExpr as
                 -- constructors is confusing
      { cid_ext     :: XCClsInstDecl pass -- TODOT
      , cid_poly_ty :: LHsSigType pass    -- Context => Class Instance-type
                                          -- Using a polytype means that the renamer conveniently
                                          -- figures out the quantified type variables for us.
